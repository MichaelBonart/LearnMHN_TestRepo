...
extern "C" {
/**
 * this function computes the gradient and score for the current MHN for a given datapoint using CUDA
 *
 * @param[in] ptheta array containing the theta entries
 * @param[in] n number of genes considered by the MHN, also number of columns/rows of theta
 * @param[in] data_point array of States, where each state represents a tumor sample
 ...
 */
int DLL_PREFIX cuda_gradient_and_score_datapoint(double *ptheta, int n, State *data_point, double *p0, double *grad_out, double *score_out) {
    // determine the maximum number of mutations present in a single tumor sample
    int mutation_num = get_mutation_num(data_point);
    ...
    // copy p0 to the GPU
    cudaMemcpy(p0_pD, p0, nx * sizeof(double), cudaMemcpyHostToDevice);
    ...
    // compute the gradient for each tumor sample
    cuda_restricted_gradient(cuda_ptheta, data_point, n, partial_grad, p0_pD, pth, q, tmp1, tmp2);

    // copy the results to the CPU
    cudaMemcpy(grad_out, partial_grad, n * n * sizeof(double), cudaMemcpyDeviceToHost);
    cudaMemcpy(score_out, &pth[(1 << mutation_num) - 1], sizeof(double), cudaMemcpyDeviceToHost);
...
int DLL_PREFIX cuda_gradient_and_score_implementation(double *ptheta, int n, State *mutation_data, int data_size, double *grad_out, double *score_out) {
    ...
    for (int i = 0; i < data_size; i++) {
        if (get_mutation_num(&mutation_data[i]) > max_mutation_num)
            max_mutation_num = get_mutation_num(&mutation_data[i]);
    }
    ...
    for (int i = 0; i < data_size; i++) {
        // initialize p0_pD with zeros and only one in the last entry
